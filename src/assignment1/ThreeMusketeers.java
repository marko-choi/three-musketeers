...
public class ThreeMusketeers {
    ...
    private static ThreeMusketeers instance;
    private MovesHistory moves = new MovesHistory();
	private AgentOperator operator = new AgentOperator();
    ...
    public void selectMode(GameMode mode) {
        switch (mode) {
            ...
            case HumanRandom -> {
                View.getInstance().setScene(SideSelectionScene.class.getName());
                SideSelectionPanel.setMode(mode);
            ...
            if (currentAgent instanceof HumanAgent) // Human move
                switch (getInputOption()) {
                    case "M":
                        ...
                        break;
                    case "U":
                        if (moves.getCurrentMoveIndex() == 0) {
    ...
    protected void move(final Agent agent) {
    	Command move = new MoveCommand(agent, this);
    	
    	operator.setCommand(move);
    	operator.operate();
    }

    public void move() {
    	this.move(this.getCurrAgent());
    }
    ...
    /**
     * Undoes a move from the moves collection.
     */
    public void undoMove() {
    	Move move = moves.undoMove();
        board.undoMove(move);
    }
    
    /*
     * Redoes a move from the moves collection.
     */
    public void redoMove() {
    	board.redo(moves.redoMove());
    }
    ...
    private String getInputOption() {
        System.out.printf("[%s] Enter 'M' to move, 'U' to undo, 'R' to redo, and 'S' to save: ", board.getTurn().getType());
        while (!scanner.hasNext("[MUSRmusr]")) {
            System.out.print("Invalid option. Enter 'M', 'U', 'R', or 'S': ");
            scanner.next();
        }
    ...
    public void setSide(GameMode mode, Piece.Type side) {
    	if (mode.equals(GameMode.HumanRandom)) {
    		musketeerAgent = side.equals(Piece.Type.MUSKETEER) ? new HumanAgent(board) : new RandomAgent(board);
            guardAgent = side.equals(Piece.Type.GUARD) ? new HumanAgent(board) : new RandomAgent(board);
    	} else if (mode.equals(GameMode.HumanGreedy)) {
    		musketeerAgent = side.equals(Piece.Type.MUSKETEER) ? new HumanAgent(board) : new GreedyAgent(board);
            guardAgent = side.equals(Piece.Type.GUARD) ? new HumanAgent(board) : new GreedyAgent(board);
    	}
    ...
    /**
     * @return Gets the board of the game.
     */
    public Board getBoard() {
    	return this.board;
    }

    public static synchronized ThreeMusketeers getInstance() {
    	if (ThreeMusketeers.instance == null) {
    		ThreeMusketeers.instance = new ThreeMusketeers();
    	}
    	return ThreeMusketeers.instance;
    }
    
    /**
     * @return Gets the list of moves made in the game.
     */
